...
export async function translateContent(content, lang) {
  ...
  // Le mot rose du pied de page n'est recoupe qu'a la condition d'avoir ete
  // recolle: laisse vide dans le Studio, il doit le rester.
  ...
  const recolle = recollerTitres(avecPied);
  ...
  if (!entrees.length) return lang === "dr" ? finir(source) : content;
  ...
  if (lang === "dr" && !process.env.GEMINI_API_KEY) return finir(source);
  ...
  try {
    ...
    if (!traductions) return lang === "dr" ? finir(source) : content;
    return finir(appliquer(source, entrees, traductions));
  } catch (error) {
    ...
    return lang === "dr" ? finir(source) : content;
  }
}
