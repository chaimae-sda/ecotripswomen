...
function lireDarija(content, entrees) {
  ...
  // Les garanties portent parfois leur propre darija, saisie directement dans
  // la fiche du voyage ou des reglages. Elle l'emporte sur la traduction
  // globale: c'est ce qui permet de personnaliser "Khelsi 400 daba" pour un
  // voyage et "Khelsi 100 daba" pour un autre, meme si le texte francais est
  // le meme.
  const appliquerDarija = (garanties) =>
    (garanties || []).map((g) => ({
      ...g,
      title: g.titleDarija?.trim() || g.title,
      text: g.textDarija?.trim() || g.text,
    }));

  const resultat = {
    ...traduit,
    offers: (traduit.offers || []).map((offre) => ({
      ...offre,
      guarantees: appliquerDarija(offre.guarantees),
    })),
    settings: {
      ...traduit.settings,
      guarantees: appliquerDarija(traduit.settings?.guarantees),
    },
  };

  return finaliser(resultat, piedRecolle);
}
