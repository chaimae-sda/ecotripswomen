...
export const IGNORER = new Set([
  ...
  "priceValue",
  "durationDays",
  // Les traductions darija saisies directement sur la garantie: ce ne sont pas
  // des textes a traduire, mais des traductions deja faites.
  "titleDarija",
  "textDarija",
...
export const SECTIONS = [
  ...
  // Cet onglet ne vient pas de Sanity mais de lib/ui.js: menus, formulaire
  // de reservation, calendrier, filtres. Les scripts de remplissage l'ajoutent.
  { name: "interface", title: "Interface du site" },
];
...
// Les phrases du site avec leur onglet et, pour l'onglet Voyages, le nom du
// voyage d'ou elles viennent: la fiche du Studio se relit alors voyage par
// voyage plutot qu'en une seule liste de soixante-douze lignes.
//
// Sans doublon et dans l'ordre de lecture. Une phrase qui apparait dans deux
// voyages ("Transport touristique confortable") n'a qu'une seule traduction:
// elle est rangee sous le premier voyage, et la corriger la corrige partout.
export function listerTextesParSection(content) {
  const { source } = preparerSource(content);
  const titres = (source.offers || []).map((offre) => offre.title || "");
  const vus = new Map();

  for (const { chemin, texte } of collecter(source)) {
    if (vus.has(texte)) continue;

    const section = sectionDe(chemin);
    // chemin vaut ["offers", 3, "program", 1, "title"]: l'index du voyage est
    // en deuxieme position.
    const voyage = section === "voyages" ? titres[chemin[1]] || "" : null;
    vus.set(texte, { section, voyage });
  }

  return [...vus].map(([texte, info]) => ({ texte, ...info }));
}
