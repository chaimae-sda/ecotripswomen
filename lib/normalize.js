...
function toOffer(offer) {
  return {
    ...
    // Engagements propres au voyage: ils remplacent les garanties generales
    // dans le bloc affiche sous le bouton Reserver (voir app/guarantees.jsx).
    // Les champs titleDarija et textDarija permettent de saisir la darija
    // directement sur la garantie, sans passer par la fiche de traduction.
    guarantees: (offer.guarantees || []).filter((g) => g?.title).map((g) => ({
      title: g.title, text: g.text || "", titleDarija: g.titleDarija || "", textDarija: g.textDarija || "",
    })),
...
export function normalize(data) {
  ...
  const settings = {
    ...
    guarantees: (s.guarantees ?? base.settings.guarantees).filter((g) => g?.title).map((g) => ({
      title: g.title, text: g.text || "", titleDarija: g.titleDarija || "", textDarija: g.textDarija || "",
    })),
