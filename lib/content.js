...
// Les dates proposees dans le formulaire de reservation. Sans date choisie
// dans le Studio, on retombe sur la date de depart, puis sur le texte affiche
// sur la carte.
function toDepartureDates(offer) {
  const choisies = toList(offer.departureDates).map(toDateLabel).filter(Boolean);
  if (choisies.length) return choisies;

  const debut = offer.startDate ? toDateLabel(offer.startDate) : null;
  if (debut) return [debut];

  return toList([offer.date]);
}
