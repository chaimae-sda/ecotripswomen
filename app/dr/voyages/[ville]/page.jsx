...
export async function generateMetadata({ params }) {
  ...
  const { offers, ui } = await getSiteContent("dr");
  ...
  return {
    ...
    alternates: {
      canonical: `/dr/voyages/${city.slug}`,
    },
    openGraph: { title, description, url: `/dr/voyages/${city.slug}`, locale: "ar_MA" },
  };
}
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            url: `${siteUrl}/dr/voyages/${city.slug}`,
            ...
            mainEntity: {
              ...
              itemListElement: city.offers.map((offer, index) => ({
                ...
                position: index + 1,
                name: offer.title,
                url: `${siteUrl}/dr/offres/${offer.slug}`,
              })),
            },
          }),
        }}
        ...
        {autres.length > 0 && (
          <nav className="city-links" aria-label={ui.villesAutres}>
            ...
            <ul>
              {autres.map((item) => (
                <li key={item.slug}>
                  <Link href={`/dr/voyages/${item.slug}`}>{item.name}</Link>
                </li>
              ))}
            </ul>
          </nav>
